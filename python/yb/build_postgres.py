...
from yb.compile_commands import (
    ...
    PG_POSTPROCESSED_COMPILE_COMMANDS_FILE_NAME,
    YB_POSTPROCESSED_COMPILE_COMMANDS_FILE_NAME,
    COMBINED_RAW_COMPILE_COMMANDS_FILE_NAME,
    YB_RAW_COMPILE_COMMANDS_FILE_NAME,
    PG_RAW_COMPILE_COMMANDS_FILE_NAME,
...
class PostgresBuilder(YbBuildToolBase):
    ...
    def write_compile_commands_file(
            ...
            rel_file_name: str) -> None:
        write_json_file(
            json_data=compile_commands,
            output_path=os.path.join(self.build_root, rel_file_name),
            description_for_log=rel_file_name)

    def write_compile_commands_files(self, pg_compile_commands_paths: List[str]) -> None:
        """
        Write various types of compilation command files across the following dimentions:
