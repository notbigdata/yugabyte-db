          ...
          while (!stop.get()) {
            try {
              ...
              for (int i = 0; i < INSERTS_PER_TXN; ++i) {
                ...
                lockIndexes.add(keyAndTypeToLockIndex(k, keyType));
                String valueAndType = v + "_" + keyType;

                AtomicInteger versionAtomic =
                    keyAndTypeToNextVersion.computeIfAbsent(keyAndType, (String dummy) ->
                    new AtomicInteger(0));
                int version = versionAtomic.get();
                if (!keyAndTypeSet.contains(keyAndType) && !valueAndTypeSet.contains(valueAndType)) {
                  keyAndTypeSet.add(keyAndType);
                  valueAndTypeSet.add(valueAndType);
                  sb.append(
                    String.format(
                      "INSERT INTO global_keys.global_akpk_one_to_one " +
                      "(key_id, key_type, key_value, modified_dtm) VALUES " +
                      "('%s', '%s', '%s', '%s') " +
                      "IF version = null OR version = %d ELSE ERROR; ",
                      k, keyType, v, simpleDateFormat.format(new Date()), version
                    )
                  );
                }
              }
...
/*
akpk@ycqlsh> select * from global_keys.global_akpk_one_to_one where key_id IN ('540513848','990644842','292457951','572588397','653903073','202603688','184398429','869276438');
key_id | key_type | key_value | modified_dtm | version
-----------+----------------+--------------+---------------------------------+---------------
184398429 | ITEMID_TO_WPID | 64AM11N6RLZB | 2020-10-21 16:02:33.016000+0000 | 1603296152926
202603688 | ITEMID_TO_WPID | 41XVSHNMBP94 | 2020-10-21 15:51:42.472000+0000 | 1603295502330
292457951 | ITEMID_TO_WPID | 3CDGFYELIVHP | 2020-10-21 15:52:06.480000+0000 | 1603295526198
 */
