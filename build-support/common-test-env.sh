...
prepare_for_running_cxx_test() {
  ...
  register_test_artifact_files "$test_log_path_prefix.*"
...
run_java_test() {
  ...
  register_test_artifact_files "$junit_xml_path" "$test_log_path" "${log_files_path_prefix}*"
...
register_test_artifact_files() {
  if [[ -n ${YB_TEST_ARTIFACT_LIST_PATH:-} ]]; then
    ...
    echo "Current contents of $YB_TEST_ARTIFACT_LIST_PATH:"
    cat "$YB_TEST_ARTIFACT_LIST_PATH"
  fi
}
