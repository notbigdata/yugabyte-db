...
#   test_log_path
#     We parse the given log file to determine PIDs that generated core files, and we append the
#     results of analyzing those core files to this log path.
process_core_file() {
  expect_num_args 0 "$@"
  expect_vars_to_be_set rel_test_binary
  "$YB_SRC_ROOT/python/yb/analyze_core_file.py" \
      --build-root "$BUILD_ROOT" \
      --rel-test-binary "$rel_test_binary" \
...
#     file at this path.
# TODO mbautin: remove this function.
process_core_file_old() {
  expect_num_args 0 "$@"
  expect_vars_to_be_set \
    rel_test_binary
