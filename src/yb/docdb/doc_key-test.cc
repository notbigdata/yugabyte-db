...
class DocKeyTest : public YBTest {
 protected:
  vector<SubDocKey> GetVariedSubDocKeys() {
    ...
    Uuid cotable_id;
    CHECK_OK(cotable_id.FromHexString("0123456789abcdef0123456789abcdef"));
    for (int has_cotable_id = 0; has_cotable_id <= 1; ++has_cotable_id) {
  ...
  for (const auto& sub_doc_key : GetVariedSubDocKeys()) {
    for (auto partial_range_key_intents : PartialRangeKeyIntents::kValues) {
      ...
      for (size_t i = 0; i < num_intents; ++i) {
        ...
        SCOPED_TRACE(Format("Generated intent bytes, partially human-readable: $0",
                            util::FormatSliceAsStr(intent.intent_key.AsSlice())));
