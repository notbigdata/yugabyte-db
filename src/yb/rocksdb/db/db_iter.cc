...
// representation into a single entry while accounting for sequence
// numbers, deletion markers, overwrites, etc.
class RegularDBIter : public Iterator {
 public:
  // The following is grossly complicated. TODO: clean it up
  // Which direction is the iterator currently moving?
  // (1) When moving forward, the internal iterator is positioned at
 ...
 private:
  ...
  void operator=(const RegularDBIter&);
};

inline bool RegularDBIter::ParseKey(ParsedInternalKey* ikey) {
  if (!ParseInternalKey(iter_->key(), ikey)) {
    status_ = STATUS(Corruption, "corrupted internal key in DBIter");
    RLOG(InfoLogLevel::ERROR_LEVEL,
        logger_, "corrupted internal key in DBIter: %s",
...
void RegularDBIter::PrevInternal() {
  if (!iter_->Valid()) {
    valid_ = false;
    return;
  }
...
// Don't use Seek(),
// because next user key will be very close
void RegularDBIter::FindNextUserKey() {
  if (!iter_->Valid()) {
    return;
  }
  ParsedInternalKey ikey;
...
// Go to previous user_key
void RegularDBIter::FindPrevUserKey() {
  if (!iter_->Valid()) {
    return;
  }
  size_t num_skipped = 0;
...
// Skip all unparseable keys
void RegularDBIter::FindParseableKey(ParsedInternalKey* ikey, Direction direction) {
  while (iter_->Valid() && !ParseKey(ikey)) {
    if (direction == kReverse) {
      iter_->Prev();
    } else {
